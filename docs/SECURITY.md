...
Static responses set a CSP and related headers. The application uses system fonts and first-party local assets only. Blob sources are allowed specifically for decrypted image, PDF, MP4, text, and DOCX previews. PDF.js processes decrypted PDFs inside the endpoint trust boundary. DOCX files are converted by the bundled Mammoth library in a dedicated browser worker with external file access and embedded style maps disabled. Archive entry counts, expanded bytes, individual entry size, generated HTML size, rendered node count, and rendered text are bounded. The UI copies only allowlisted semantic elements and embedded raster data images into the document, removing active content, event handlers, remote resources, and non-fragment links. Bundled Apache-2.0 PP-OCRv5 models from PaddlePaddle and ONNX Runtime WebAssembly provide local text layers for images and scanned PDF pages; preview pixels and recognized plaintext are not transmitted or persisted. The CSP grants `'wasm-unsafe-eval'` for WebAssembly compilation but does not grant JavaScript `'unsafe-eval'`. No analytics or third-party runtime script is required.
