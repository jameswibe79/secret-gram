...
Static responses set a CSP and related headers. The application uses system fonts and first-party local assets only. Blob sources are allowed specifically for decrypted image and PDF previews. PDF.js processes decrypted PDFs inside the endpoint trust boundary. Bundled Apache-2.0 PP-OCRv5 models from PaddlePaddle and ONNX Runtime WebAssembly provide optional local text layers for images and scanned PDF pages; preview pixels and recognized plaintext are not transmitted or persisted. The CSP grants `'wasm-unsafe-eval'` for WebAssembly compilation but does not grant JavaScript `'unsafe-eval'`. No PDF content, image content, or OCR output is sent to an external viewer, CDN, or model provider, and no analytics or third-party runtime script is required.
