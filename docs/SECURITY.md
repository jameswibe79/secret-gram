...
Static responses set a CSP and related headers. The application uses system fonts and first-party local assets only. Blob sources are allowed specifically for decrypted image, PDF, MP4, text, DOCX, and XLSX previews. PDF.js processes decrypted PDFs inside the endpoint trust boundary. Before DOCX rendering, a dedicated browser worker validates archive structure and bounds both declared and actual streamed expansion. The bundled docx-preview renderer reconstructs Word layout in an isolated shadow tree with active altChunks and comments disabled. Generated stylesheet text is stripped of remote URLs and installed as constructable stylesheets, preserving document formatting without adding `'unsafe-inline'` to the CSP. Embedded images and fonts become local data URLs; non-embedded image sources, active elements, remote CSS resources, and unsafe link protocols are removed. XLSX parsing also runs in a dedicated browser worker. It validates ZIP structure and paths, rejects document types and external worksheet relationships, bounds declared and actual XML expansion, and caps sheets, rows, columns, cells, styles, and shared strings. Its inert table renderer uses cached values and never executes formulas, macros, hyperlinks, or embedded objects. Archive entry counts, expanded bytes, individual entry size, and file size are bounded. Bundled Apache-2.0 PP-OCRv5 models from PaddlePaddle and ONNX Runtime WebAssembly provide local text layers for images and scanned PDF pages; preview pixels and recognized plaintext are not transmitted or persisted. The CSP grants `'wasm-unsafe-eval'` for WebAssembly compilation but does not grant JavaScript `'unsafe-eval'`. No analytics or third-party runtime script is required.
